...
def вложения(текст, куда):
    """Скачать всё, на что ссылается тело, и вернуть карту адрес -> локальный путь."""
    карта = {}
...
def main():
    ...
    доска, файлов, вложений_всего = [], 0, [0]
    for i in issues:
        ...
        карта = вложения(весь_текст, os.path.join(КОРЕНЬ, "files"))
    ...
    with open(os.path.join(КОРЕНЬ, "README.md"), "w", encoding="utf-8") as f:
        f.write(
            ...
            % (сводка["выгружено"], сводка["постов"], сводка["ответов"], вложений_всего[0]))

    print("выгружено: постов %d, ответов %d, вложений %d, файлов %d" % (
        сводка["постов"], сводка["ответов"], вложений_всего[0], файлов + 2))
